...
struct E2ESession {
    ...
    std::chrono::steady_clock::time_point previous_expires_at;
};
...
bool send_chat_message(int sockfd, const std::string& peer, const std::string& content) {
    ...
    if (has_e2e) {
        ...
        return send_encrypted(sockfd, "MSG|" + peer + "|__E2E_MSG__" + std::to_string(generation) + "|" + bytes_to_hex(blob));
    } else {
...
bool i_am_initiator(const std::string& peer) {
...
void handle_e2e_init(int sockfd, const std::string& peer, const std::string& payload) {
    ...
    int target_gen = std::atoi(payload.substr(0, sep).c_str());
    std::string their_pub_hex = payload.substr(sep + 1);

    {
        std::lock_guard<std::mutex> lock(e2e_mutex);
        auto it = e2e_sessions.find(peer);
        if (it != e2e_sessions.end() && target_gen <= it->second.generation) {
            return;
        }
    }
...

    {
        ...
        if (session.generation > 0) {
            std::memcpy(session.previous_key, session.key, 32);
            session.previous_generation = session.generation;
            session.has_previous = true;
            session.previous_expires_at = std::chrono::steady_clock::now() + std::chrono::seconds(10);
        }
...
void handle_e2e_ack(const std::string& peer, const std::string& payload) {
    size_t sep = payload.find('|');
    if (sep == std::string::npos) return;
    int target_gen = std::atoi(payload.substr(0, sep).c_str());
    std::string their_pub_hex = payload.substr(sep + 1);
    ...
    DHKeyPair my_kp;
    {
        ...
        if (it == pending_e2e_keypairs.end() || it->second.target_generation != target_gen) {
            std::cout << "\n[E2E] Received unexpected/stale ACK from " << peer << ". Ignoring.\n> " << std::flush;
            return;
        }
        my_kp = it->second.kp;
...
void initiate_e2e(int sockfd, const std::string& peer, int target_generation) {
    DHKeyPair my_kp = dh_generate_keypair(g_e2e_p, g_e2e_g, g_e2e_ctx);
    {
        std::lock_guard<std::mutex> lock(e2e_mutex);
        pending_e2e_keypairs[peer] = PendingRekey{my_kp, target_generation};
    }
    send_encrypted(sockfd, "MSG|" + peer + "|__E2E_INIT__" + std::to_string(target_generation) + "|" + dh_bn_to_hex(my_kp.pub));
}

void rekey_timer_loop(int sockfd, std::string peer) {
    while (running) {
        std::this_thread::sleep_for(std::chrono::seconds(REKEY_INTERVAL_SECONDS));
        if (!running) break;
        if (!i_am_initiator(peer)) continue;

        int current_gen;
        {
            std::lock_guard<std::mutex> lock(e2e_mutex);
            auto it = e2e_sessions.find(peer);
            if (it == e2e_sessions.end()) continue;
            current_gen = it->second.generation;
        }
        initiate_e2e(sockfd, peer, current_gen + 1);
    }
}
    ...
    while (running) {
        ...
        if (type == "MSG" && parts.size() >= 3) {
            ...
            if (content.rfind("__E2E_INIT__", 0) == 0) {
                ...
                std::string rest = content.substr(11);
                size_t sep = rest.find('|');
                if (sep == std::string::npos) continue;
                int msg_gen = std::atoi(rest.substr(0, sep).c_str());
                std::string blob = hex_to_bytes(rest.substr(sep + 1));

                bool found = false;
                unsigned char use_key[32];
                {
                    std::lock_guard<std::mutex> lock(e2e_mutex);
                    auto it = e2e_sessions.find(sender);
                    if (it != e2e_sessions.end()) {
                        if (msg_gen == it->second.generation) {
                            found = true;
                            std::memcpy(use_key, it->second.key, 32);
                        } else if (it->second.has_previous && msg_gen == it->second.previous_generation
                                   && std::chrono::steady_clock::now() < it->second.previous_expires_at) {
                            found = true;
                            std::memcpy(use_key, it->second.previous_key, 32);
                        }
                    }
                }

                if (!found) {
                    std::cout << "\n[E2E] Received a message from " << sender
                              << " with an unknown/expired key generation (" << msg_gen << "). Ignoring.\n> " << std::flush;
                } else {
                    std::string plaintext;
                    if (aes_gcm_decrypt(use_key, blob, plaintext)) {
                        std::cout << "\n[" << sender << " (E2E gen " << msg_gen << ")] " << plaintext << "\n> " << std::flush;
                    } else {
